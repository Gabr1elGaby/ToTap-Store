                ...
                <div id="topup-paid-card" class="{{ $isPaid ? '' : 'hidden' }} bg-white dark:bg-gray-900 rounded-3xl p-6 sm:p-8 mb-6 text-center border-2 {{ $isSuccess ? 'border-emerald-500/30 dark:border-emerald-500/20' : 'border-blue-500/30 dark:border-blue-500/20' }} shadow-lg space-y-6">
                    @if($isSuccess)
                        ...
                        @if(!empty($transaction->provider_sn))
                            @if(\App\Services\VipResellerService::isPendingSn($transaction->provider_sn))
                                <div class="bg-blue-500/10 dark:bg-blue-950/40 border-2 border-blue-500/40 rounded-2xl p-4 sm:p-5 shadow-sm max-w-lg mx-auto flex items-center gap-3.5 text-left">
                                ...
                                <div class="bg-amber-500/10 dark:bg-amber-950/40 border-2 border-amber-500 p-5 sm:p-6 rounded-2xl text-left space-y-4 max-w-lg mx-auto shadow-lg">
                                    ...
                                    <div class="space-y-3">
                                        ...
                                        <div class="flex flex-wrap items-center gap-2 pt-1">
                                            ...
                                            @if($detectedLink)
                                            <a href="{{ $detectedLink }}" target="_blank" class="px-4 py-2.5 text-xs font-black text-white bg-blue-600 hover:bg-blue-700 active:scale-95 rounded-xl shadow-md transition whitespace-nowrap cursor-pointer flex items-center gap-1.5 shrink-0">

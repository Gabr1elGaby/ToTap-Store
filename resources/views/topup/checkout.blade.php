                                ...
                                <div>
                                    ...
                                    <div class="p-3 bg-slate-900 dark:bg-slate-950 rounded-xl border-2 border-amber-500/40 font-mono text-xs font-bold {{ $isUrl ? 'text-blue-300' : ($isPass ? 'text-emerald-400' : ($isProfile ? 'text-purple-300' : 'text-amber-300')) }} select-all flex items-center justify-between gap-3 shadow-inner">
                                        ...
                                        <div class="flex items-center gap-1.5 shrink-0">
                                            <button type="button" onclick="navigator.clipboard.writeText('{{ addslashes($val) }}'); alert('Link disalin!');" class="px-2.5 py-1.5 text-xs font-black text-white bg-slate-700 hover:bg-slate-600 active:scale-95 rounded-lg shadow-md transition whitespace-nowrap cursor-pointer flex items-center gap-1">
                                                <i class="fas fa-copy"></i>
                                            </button>
                                            <a href="{{ $val }}" target="_blank" class="px-3 py-1.5 text-xs font-black text-white bg-blue-600 hover:bg-blue-700 active:scale-95 rounded-lg shadow-md transition whitespace-nowrap cursor-pointer flex items-center gap-1.5">
                                                <i class="fas fa-external-link-alt"></i> Buka Link
                                            </a>
                                        </div>
                                        @else
                                        <span class="break-all leading-relaxed">{{ $val }}</span>
                                        <button type="button" onclick="navigator.clipboard.writeText('{{ addslashes($val) }}'); alert('{{ $label }} berhasil disalin!');" class="px-3 py-1.5 text-xs font-black text-white {{ $isPass ? 'bg-emerald-600 hover:bg-emerald-700' : ($isProfile ? 'bg-purple-600 hover:bg-purple-700' : 'bg-amber-600 hover:bg-amber-700') }} active:scale-95 rounded-lg shadow-md transition whitespace-nowrap cursor-pointer flex items-center gap-1.5 shrink-0">

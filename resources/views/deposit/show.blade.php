<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            <a href="{{ route('deposit.index') }}" class="inline-flex items-center gap-2 px-4 py-2 bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-200 font-bold text-xs rounded-xl transition shadow-sm border border-gray-200 dark:border-gray-700">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-3xl p-6 sm:p-8 border border-gray-200 dark:border-gray-700 shadow-2xl space-y-6 text-center">
                ...
                @if($deposit->status === 'success' || $deposit->status === 'paid')
                    ...
                    <div>
                        ...
                        <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Saldo akun Anda telah bertambah otomatis.</p>
                    </div>
                ...
                @else
                    ...
                    <div>
                        ...
                        <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">No. Deposit: <span class="font-mono font-bold text-gray-800 dark:text-gray-200">{{ $deposit->id }}</span></p>
                    </div>

                    <!-- Nominal Tagihan -->
                    <div class="p-4 rounded-2xl bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700">
                        <div class="text-xs text-gray-500 dark:text-gray-400 font-medium">Nominal Saldo yang Masuk:</div>
                    ...
                    <!-- Kotak QR Code -->
                    <div class="bg-white p-6 rounded-3xl border-2 border-dashed border-gray-300 shadow-md inline-block max-w-full">
                        <div class="flex items-center justify-between mb-3 px-2">
                            <img src="https://upload.wikimedia.org/wikipedia/commons/a/a2/Logo_QRIS.svg" alt="QRIS" class="h-6 object-contain">
                            <span class="text-[10px] font-black text-gray-400 uppercase tracking-wider">NMID: ID1026577601523</span>
                        </div>
                        
                        @php
                            $qrImgUrl = 'https://api.qrserver.com/v1/create-qr-code/?size=300x300&margin=10&data=' . urlencode($qrisString);
                        @endphp
                        <img src="{{ $qrImgUrl }}" alt="QRIS Deposit" class="w-64 h-64 mx-auto rounded-xl object-contain border border-gray-100 shadow-inner">

                        <div class="text-center mt-3">
                            <div class="text-[11px] font-black text-gray-800">ToTap Store, Gaming</div>
                            <div class="text-[10px] text-gray-400">QRIS Dinamis • Nominal Pas Otomatis</div>
                        </div>
                    </div>

                    <!-- Petunjuk Singkat -->
                    <div class="text-left text-xs bg-indigo-50/50 dark:bg-indigo-950/30 p-4 rounded-2xl border border-indigo-200 dark:border-indigo-800 text-indigo-950 dark:text-indigo-200 space-y-1.5 font-medium">
                    ...
                    <div class="space-y-3 pt-2">
                        <button type="button" 
                                ...
                                id="btn-check-status"
                                class="w-full py-3.5 bg-emerald-500 hover:bg-emerald-400 text-gray-950 font-black text-sm rounded-2xl shadow-lg transition flex items-center justify-center gap-2 cursor-pointer">
                            <svg class="w-4 h-4 animate-spin hidden" id="spinner-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
                            <span id="btn-check-text">Saya Sudah Bayar (Cek Saldo)</span>
                        </button>
                        ...
                        <a href="https://wa.me/6281234567890?text=Halo%20Admin%20ToTap%20Store,%20saya%20sudah%20melakukan%20isi%20saldo%20dengan%20nomor%20deposit:%20{{ $deposit->id }}%20sebesar%20Rp{{ number_format($deposit->amount, 0, ',', '.') }}" 
                           target="_blank" 
                           class="w-full py-3 bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 text-gray-700 dark:text-gray-200 font-bold text-xs rounded-2xl transition flex items-center justify-center gap-2">

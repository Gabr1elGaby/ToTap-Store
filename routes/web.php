...
Route::middleware(['auth', 'verified', 'role:superadmin'])->prefix('admin')->name('admin.')->group(function () {
    ...
    Route::post('games/sync-balance', function() {
        try {
    ...
    })->name('games.sync-balance');

    Route::post('games/sync-product-status', function() {
        set_time_limit(300);
    ...
    })->name('games.sync-product-status');

    Route::post('games/{game}/sync-product-status', function(\App\Models\Game $game) {
        set_time_limit(180);
